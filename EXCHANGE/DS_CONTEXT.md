...
**Версия:** 2.8 от 25.09.2026 (после DS_063–DS_067 + Уточнения, DS_072a–DS_072e, DS_073, DS_075).
...
Дополнение DS_075: в `save_scan_only_log` после блока «Правил в ignore» идёт итоговая группа: `Всего проблем: N`, затем те же три счётчика — `Прогноз исправлений: X`, `Исправлено: 0`, `В AI: Z` (тот же порядок, что в `scan_report_*`, п. 8.2), затем `Всего файлов: M`. Строка `Спрогнозировано исправлений:` (DS_053_Уточнение_4/5) выводится **не** была — актуальная метка `Прогноз исправлений:`. Топ-файлы — по тем же условиям (`log_level == "Подробный"` и файлов ≥ `report_stats_min_files`).
