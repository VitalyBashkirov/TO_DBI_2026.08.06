#!/usr/bin/env python3
"""
АРМ "Адаптация под DBI" v25.1.001
Графический интерфейс для миграции PLPlus кода на DBI
Обновлённая версия с главным меню и улучшенным интерфейсом
Версия: 2025 Q1 (январь-март), версия 001
"""
...
class DBIMigrationApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("АРМ 'Адаптация под DBI' v06")
        ...
        # Выбранные правила
        self.selected_rules = {}
        self.rule_checkboxes = {}  # Для доступа к виджетам
        ...
        # Привязка событий для сброса прогресса
    ...
    def _create_widgets(self):
        ...
        # Секция 1 - Пути (объединённая)
        paths_frame = ttk.LabelFrame(scrollable_frame, text="1. Пути к каталогам", padding="10")
        ...
        ttk.Entry(paths_frame, textvariable=self.source_dir_var, width=70).grid(row=0, column=1, padx=5, sticky=tk.EW)
        ...
        ttk.Entry(paths_frame, textvariable=self.result_dir_var, width=70).grid(row=0, column=4, padx=5, sticky=tk.EW)
        ...
        # Секция 2 - Настройки сканирования
        scan_frame = ttk.LabelFrame(scrollable_frame, text="2. Настройки сканирования", padding="10")
        scan_frame.pack(fill=tk.X, padx=10, pady=5)
        
        ttk.Label(scan_frame, text="Шаблон исходных файлов:").grid(row=0, column=0, sticky=tk.W, padx=5)
        ttk.Entry(scan_frame, textvariable=self.file_pattern_var, width=40).grid(row=0, column=1, padx=5, sticky=tk.W)
        ttk.Checkbutton(scan_frame, text="Рекурсивно", variable=self.scan_recursive_var).grid(row=0, column=2, padx=20, sticky=tk.W)
        
        # Секция 3 - Рубрикатор
        rules_frame = ttk.LabelFrame(scrollable_frame, text="3. Рубрикатор. Выбор корректировок по документации", padding="10")
        rules_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)
        
        # Загрузка файлов рубрикатора
        self._load_rubricator_files()
        
        # Создаём Treeview для отображения правил
        rules_tree_frame = ttk.Frame(rules_frame)
        rules_tree_frame.pack(fill=tk.BOTH, expand=True)
        
        # Treeview с прокруткой
        tree_scroll_y = ttk.Scrollbar(rules_tree_frame, orient=tk.VERTICAL)
        tree_scroll_x = ttk.Scrollbar(rules_tree_frame, orient=tk.HORIZONTAL)
        ...
        # Настройка колонок
        self.rules_tree['columns'] = ('code', 'name', 'selected')
        self.rules_tree.column('#0', width=30, minwidth=30)
        ...
        # Заголовки
        self.rules_tree.heading('#0', text='', anchor=tk.W)
        self.rules_tree.heading('code', text='Код файла', anchor=tk.W)
        self.rules_tree.heading('name', text='Наименование', anchor=tk.W)
        ...
        # Секция 4 - Логирование
        log_frame = ttk.LabelFrame(scrollable_frame, text="4. Логирование", padding="10")
        log_frame.pack(fill=tk.X, padx=10, pady=5)
        
        ttk.Label(log_frame, text="Уровень:").grid(row=0, column=0, sticky=tk.W, padx=5)
        ttk.Combobox(log_frame, textvariable=self.log_level_var, 
                    values=["Минимальный", "Подробный"], 
                    state="readonly", width=20).grid(row=0, column=1, padx=5, sticky=tk.W)
        
        # Секция 5 - Опции вывода
        output_frame = ttk.LabelFrame(scrollable_frame, text="5. Опции вывода", padding="10")
        output_frame.pack(fill=tk.X, padx=10, pady=5)
        
        ttk.Checkbutton(output_frame, text="Только модифицированные файлы", 
                       variable=self.only_modified_var).grid(row=0, column=0, sticky=tk.W, padx=5)
        ttk.Checkbutton(output_frame, text="Сохранить структуру каталогов", 
        ...
        # Индикатор выполнения (под журналом)
    ...
    def _populate_rules_tree(self):
        ...
        for code, description in rules.items():
            ...
            # Вставляем в дерево
            item_id = self.rules_tree.insert('', tk.END, text='📄', 
    ...
    def _bind_parameter_events(self):
        ...
        self.source_dir_var.trace_add('write', self._on_parameter_change)
        self.result_dir_var.trace_add('write', self._on_parameter_change)
        self.file_pattern_var.trace_add('write', self._on_parameter_change)
        self.log_level_var.trace_add('write', self._on_parameter_change)
        self.scan_recursive_var.trace_add('write', self._on_parameter_change)
    
    def _on_parameter_change(self, *args):
        """Сброс прогресса при изменении параметров"""
        self.progress.config(value=0)
        self.progress_label.config(text="0%")
        self.log("Параметры изменены. Прогресс сброшен.", 'debug')
    ...
    def browse_source(self):
        ...
        if directory:
            old_value = self.source_dir_var.get()
    ...
    def browse_result(self):
        ...
        directory = filedialog.askdirectory()
        if directory:
            old_value = self.result_dir_var.get()
...
def main():
    ...
    print("Запуск АРМ 'Адаптация под DBI' v06...")
    root = tk.Tk()
